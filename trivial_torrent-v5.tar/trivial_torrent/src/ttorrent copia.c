...
//FUNCIONES !!!!!
static void init_buffer(uint8_t buffer[RAW_MESSAGE_SIZE]) 
{
...
static void insert_num_block(uint8_t buffer[RAW_MESSAGE_SIZE], uint64_t block_number) //pasar BLOCK_NUMBER to buffer
{
...
static uint32_t recuperar_Magic_Number(const uint8_t buffer[RAW_MESSAGE_SIZE])
{
...
static uint64_t recuperar_Block_Number(const uint8_t buffer[RAW_MESSAGE_SIZE])
{
    //[5,12]
...
static void copiarBuffer(const uint8_t bufferRecv[RAW_MESSAGE_SIZE], uint8_t bufferSend[RAW_MESSAGE_SIZE])
{
...
int main(int argc, char** argv) {
	...
	if (argc < 3) {
		...
		//servaddr.sin_addr.s_addr = htons( (uint16_t) "127.0.0.1"); //IP_addr
	...
	else
	{
		...
		for(;;){
			...
			//POLL CORRECTO
			size_fds = nfds;
			for (nfds_t i = 0; i < size_fds; i++) //Buscando fd leibles:
			{
				...
				if (fds[i].fd == sockfd) //SOCKET PRINCIPAL (ACCEPT)
				{
				...
				else //SOCKETS PARA CONEXIONES ENTRANTES 
				{
					fin_conexion = 0;
					//PETICIÓN
