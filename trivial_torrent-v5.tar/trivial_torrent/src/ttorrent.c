...
//FUNCIONES 
static void init_buffer(uint8_t buffer[RAW_MESSAGE_SIZE])  //INICIALIZAR BUFFER PARA ENVIAR
{
...
static void insert_num_block(uint8_t buffer[RAW_MESSAGE_SIZE], uint64_t block_number) //INSERTAMOS NUMERO DE BLOQUE AL BUFFER PARA ENVIAR
{
...
static uint32_t recuperar_Magic_Number(const uint8_t buffer[RAW_MESSAGE_SIZE]) //RECUPERAMOS MAGIC NUMBER DE BUFFER RECIBIDO
{
...
static uint64_t recuperar_Block_Number(const uint8_t buffer[RAW_MESSAGE_SIZE]) //RECUPERAMOS NUMERO DE BLOQUE DE BUFFER RECIBIDO
{
...
int main(int argc, char** argv) {
	...
	if (argc < 3) {
		...
		servaddr.sin_addr.s_addr = (in_addr_t)  inet_addr("127.0.0.1"); //IP_addr
		...
		while (p < torrent.peer_count && blocks_complets < torrent.block_count)   //RECORREMOS LOS PUERTOS; p => PEER_NUMBER
		{
			...
			{
				log_printf(LOG_INFO, "\nPeer #%d ", p);
				perror("Error al conectarse al server.");
			...
			else
			{
				...
				for (uint64_t b = 0; b < torrent.block_count; b++) //RECORREMOS LOS BLOQUES; b => BLOCK_NUMBER
				{
					if (!torrent.block_map[b]) //SI NO TENEMOS EL BLOQUE EN MEMORIA:
					{
						...
						insert_num_block(buffer_send, b); //INSERTAR NUM_BLOCK EN BUFFER
                        
                        //PEDIMOS BLOQUE AL SERVER
						...
						if (buffer_recv[4] == MSG_RESPONSE_OK) //SERVER TIENE EL BLOQUE
						{
	...
	else
	{
		...
		//REMOVEMOS LA EXTENSIÓN DEL FICHERO
		char downloaded_file[50];
		int flag = 0;
		...
		struct sockaddr_in servaddr;
		servaddr.sin_addr.s_addr = (in_addr_t) inet_addr("127.0.0.1");
		servaddr.sin_family = AF_INET;
		...
		//INICIALIZAMOS LA LISTA FDS CON EL FD DEL PROGRAMA PRINCIPAL
		fds[0].fd = sockfd;
		fds[0].events = POLLIN;
		...
		for(;;)
		{
			...
			//POLL EJECUTADO CORRECTAMENTE
			size_fds = nfds;
			for (nfds_t i = 0; i < size_fds; i++) //LEEMOS TODOS LOS FD DE LA LISTA FDS
			{
				...
				if (fds[i].fd == sockfd) //SOCKET QUE EJECUTA ACCEPT (PRINCIPAL)
				{
					log_printf(LOG_INFO, "\nNueva conexión entrante.");
					sockfd_1 = accept(sockfd, NULL, NULL); 	
					
					if (sockfd_1 > 0)
					{				
						log_printf(LOG_INFO, "Conexión entrante aceptada.\n");

						//AÑADIMOS NUEVO SOCKET A LA LISTA FDS
						fds[nfds].fd = sockfd_1;
						fds[nfds].events = POLLIN;
						nfds++;
					}
				}
				else //SOCKETS PARA ATENDER CONEXIONES ENTRANTES (SECUNDARIOS) 
				{
					fin_conexion = 0;
					//RECIBIMOS PETICIÓN DEL CLIENTE
					...
					else if (!res_recv) //DEJAMOS DE RECIBIR DEL CLIENTE, DEBEMOS CERRAR EL SOCKET
					{
						log_printf(LOG_INFO, "\n      Conexión cerrada :)\n");
						fin_conexion = 1;
					}
					...
					{
						//TRATAMOS LA PETICIÓN DEL CLIENTE
						if (buffer_recv[4] == MSG_REQUEST ) 
						{
							//RECUPERAMOS INFORMACIÓN DEL BUFFER RECIBIDO
							block_number_recv = recuperar_Block_Number(buffer_recv);
							magic_number_recv = recuperar_Magic_Number(buffer_recv);
							
							for(int j = 0; j < RAW_MESSAGE_SIZE; j++)
							{
								buffer_send[j] = buffer_recv[j];
							}
						...
						else //NO ENTENDEMOS EL MENSAJE DEL CLIENTE
						{
							perror("¡Mensaje recibido incorrecto!\n"); 
							exit(0);
						}
					}
					//CERRAMOS EL SOCKET SECUNDARIO: CERRAMOS CONEXIÓN CON EL CLIENTE
					if(fin_conexion)
					{
						if(close(fds[i].fd) < 0)
						{
		...
		if(close(sockfd) < 0) //CERRAMOS EL SOCKET PRINCIPAL: SE TERMINA EL PROGRAMA
		{
			log_printf(LOG_INFO, "¡Fallo en el close!");
			exit(EXIT_FAILURE);
		}
